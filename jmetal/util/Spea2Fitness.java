	...
	public SolutionSet environmentalSelection(int size) {
		...
		while (i < solutionSet_.size()) {
			if (solutionSet_.get(i).getFitness() < 1.0) {
				...
				/**
				 * Modification by CZ: do not include duplicate solutions
				 */
				boolean duplicate = false;
				for (int k1 = 0; k1 < aux.size(); k1++) {
					int nrEqual = 0;
					for (int k2 = 0; k2 < solutionSet_.get(i).numberOfObjectives(); k2++) {
						if (solutionSet_.get(i).getObjective(k2) == aux.get(k1).getObjective(k2)) {
							nrEqual++;
						}
					}
					if (nrEqual == solutionSet_.get(i).numberOfObjectives()) {
						duplicate = true;
				...
				/** Original JMetal implementation */
				// aux.add(solutionSet_.get(i));
				// solutionSet_.remove(i);
				/** End of original JMetal implementation */
			} else {
				i++;
			} // if
		} // while
		...
		if (aux.size() < size) {
			...
			for (i = 0; i < remain; i++) {
				/** Modification made by CZ */
				if (i < solutionSet_.size()) {
					aux.add(solutionSet_.get(i));
				}
				/** End of modification made by CZ */
		...
		while (aux.size() > size) {
			...
			while (iterator.hasNext()) {
				...
				if (dn.get(0).getDistance() < minDistance) {
					...
					if ((k < distanceList.get(toRemove).size())
							&& (dn.get(k).getDistance() < distanceList.get(toRemove).get(k).getDistance())) {
						toRemove = i;
					} // if
					/** End of modification made by CZ */
